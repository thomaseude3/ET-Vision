...
class OCR_type_produit(QDialog):
    new_btn_type_produit = QtCore.pyqtSignal(str)

    def __init__(self, image1, image1_path):
        super(OCR_type_produit, self).__init__()
    ...
    def initUI(self, image1):
        ...
        title_label = QLabel("Type Produit", self)
    ...
    def perform_ocr(self):
        image_produit = cv2.imread("acquisition_image/type_produit.png")

        self.texte.append(pytesseract.image_to_string(image_produit).strip())

        print(self.texte)
        if self.texte == ['']:
            self.new_btn_type_produit.emit(self.btn_rouge)
        else:
            self.new_btn_type_produit.emit(self.btn_vert)
    ...
    def saveSelectedArea(self, pixmap, rect):
        ...
        # Sauvegarder la zone sélectionnée dans une image
        selected_area.save('acquisition_image/type_produit.png')
        type_prod = cv2.imread('acquisition_image/type_produit.png')
        inverted_image_prod_type = cv2.bitwise_not(type_prod)
        cv2.imwrite('acquisition_image/type_produit.png', inverted_image_prod_type)
