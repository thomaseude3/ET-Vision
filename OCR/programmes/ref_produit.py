...
class OCR_ref_produit(QDialog):
    new_btn_ref_produit = QtCore.pyqtSignal(str)

    def __init__(self, image1, image1_path):
        super(OCR_ref_produit, self).__init__()
        self.btn_vert = """
    ...
    def initUI(self, image1):
        ...
        title_label = QLabel("Référence Produit", self)
    ...
    def perform_ocr(self):
        image_produit = cv2.imread("acquisition_image/ref_produit.png")
        ...
        if self.texte == ['']:
            self.new_btn_ref_produit.emit(self.btn_rouge)
        else:
            self.new_btn_ref_produit.emit(self.btn_vert)
    ...
    def saveSelectedArea(self, pixmap, rect):
        ...
        # Sauvegarder la zone sélectionnée dans une image
        selected_area.save('acquisition_image/ref_produit.png')
        ref_prod = cv2.imread('acquisition_image/ref_produit.png')
        inverted_image_prod_ref = cv2.bitwise_not(ref_prod)
        cv2.imwrite('acquisition_image/ref_produit.png', inverted_image_prod_ref)
