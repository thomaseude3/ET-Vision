from PyQt6.QtWidgets import QDialog, QGraphicsScene, QGraphicsView, QGraphicsPixmapItem, QGraphicsRectItem, QWidget, \
...
class OCR_ref_etiquette(QDialog):
    new_btn_ref_eti = QtCore.pyqtSignal(str)

    def __init__(self, image1, image1_path):
        super(OCR_ref_etiquette, self).__init__()
    ...
    def initUI(self, image1):
        ...
        title_label = QLabel("Référence Étiquette", self)
    ...
    def perform_ocr(self):
        image_produit = cv2.imread("acquisition_image/ref_eti.png")

        self.texte.append(pytesseract.image_to_string(image_produit).strip())

        print(self.texte)
        if self.texte == ['']:
            self.new_btn_ref_eti.emit(self.btn_rouge)
        else:
            self.new_btn_ref_eti.emit(self.btn_vert)
    ...
    def saveSelectedArea(self, pixmap, rect):
        ...
        # Sauvegarder la zone sélectionnée dans une image
        selected_area.save('acquisition_image/ref_eti.png')
